# MeetFlow: Workflow & Architecture

This document outlines the core workflows, data flow, and networking architecture of the MeetFlow application.

## 1. Application Entry & Identity
When a user visits MeetFlow, they land on `JoinRoom.jsx`. 
- **Identity Persistence**: The app attempts to fetch a persistent `uid` from `localStorage` using the pattern `meetflow_uid_{roomId}`. If one doesn't exist, a new UUID is generated. This allows users to refresh the page without losing their identity or host privileges.
- **Room Entry**: The user enters their Name and the Room ID, which routes them to `/room/:id`.

## 2. Host Election & Waiting Room Lifecycle
MeetFlow uses Firestore to manage room state in a serverless manner.
...
Unlike traditional client-server video apps (which use an SFU/SFU), MeetFlow uses a **Full Mesh Architecture**. Every participant establishes a direct P2P connection with every other participant.
...
- **Local Media File Sharing**:
  To bypass strict OS/browser audio capture limitations (like sharing a VLC window without sound on Windows), MeetFlow supports native file injection.
...
Properly releasing camera and microphone hardware is critical. MeetFlow utilizes strict unmount paradigms:
